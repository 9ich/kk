...
#ifdef STANDALONE
  #define PRODUCT_NAME				"konkrete"
  #define BASEGAME					"konkrete"
...
//  #define STEAMPATH_APPID         ""
  #define GAMENAME_FOR_MASTER		"konkrete"	// must NOT contain whitespace
...
#define Com_Scale(x, min, max, newmin, newmax)	((newmin) + ((newmax)-(newmin)) * (float)((x)-(min)) / (float)((max)-(min)))
...
// weapon slots -- primary, secondary, and grappling hook
#define WS_NUMSLOTS	3
...
typedef struct playerState_s {
	...
	int			weapon[WS_NUMSLOTS];			// copied to entityState_t->weapon
	int			weaponstate[WS_NUMSLOTS];
...
typedef struct usercmd_s {
	...
	int				angles[3];
	int 			buttons;
	byte			weapon[WS_NUMSLOTS];           // weapon 
