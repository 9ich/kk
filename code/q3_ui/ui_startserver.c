...
StartServer_Update(void)
{
	...
	for(; i<MAX_MAPSPERPAGE; i++){
		s_startserver.mappics[i].generic.flags &= ~QMF_HIGHLIGHT;
		s_startserver.mappics[i].generic.name = nil;
		s_startserver.mappics[i].shader = 0;
...
StartServer_MenuInit(void)
{
	...
	StartServer_GametypeEvent(nil, QM_ACTIVATED);
}
...
static void
ServerOptions_PlayerNameEvent(void* ptr, int event)
{
	int n;
...
static void
ServerOptions_InitBotNames(void)
{
	int count;
	int n;
...
UI_BotSelectMenu_UpdateGrid(void)
{
	...
	for(i = 0; i < (PLAYERGRID_ROWS * PLAYERGRID_COLS); i++, j++){
		if(j < botSelectInfo.numBots){
			...
			// dead slot
			botSelectInfo.pics[i].generic.name = nil;
			botSelectInfo.picbuttons[i].generic.flags |= QMF_INACTIVE;
...
static void
UI_BotSelectMenu_Init(char *bot)
{
	int i, j, k;
	int x, y;
