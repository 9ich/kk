...
void
_UI_DrawSides(float x, float y, float w, float h, float size)
{
...
void
_UI_DrawTopBottom(float x, float y, float w, float h, float size)
{
	UI_AdjustFrom640(&x, &y, &w, &h);
...
*/
void
_UI_DrawRect(float x, float y, float width, float height, float size, const float *color)
{
...
int
Text_Width(const char *text, float scale, int limit)
{
	int count, len;
	float out;
	glyphInfo_t *glyph;
...
int
Text_Height(const char *text, float scale, int limit)
{
	int len, count;
	float max;
	glyphInfo_t *glyph;
...
void
Text_PaintChar(float x, float y, float width, float height, float scale, float s, float t, float s2, float t2, qhandle_t hShader)
{
	float w, h;
	w = width * scale;
	h = height * scale;
...
void
Text_Paint(float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style)
{
	int len, count;
	vec4_t newColor;
	glyphInfo_t *glyph;
...
void
Text_PaintWithCursor(float x, float y, float scale, vec4_t color, const char *text, int cursorPos, char cursor, int limit, int style)
{
	int len, count;
	vec4_t newColor;
...
static void
Text_Paint_Limit(float *maxX, float x, float y, float scale, vec4_t color, const char* text, float adjust, int limit)
{
	int len, count;
	vec4_t newColor;
	glyphInfo_t *glyph;
...
void
UI_DrawCenteredPic(qhandle_t image, int w, int h)
{
	int x, y;
	x = (SCREEN_WIDTH - w) / 2;
	y = (SCREEN_HEIGHT - h) / 2;
...
char *defaultMenu = nil;

char *
GetMenuBuffer(const char *filename)
{
	int len;
	fileHandle_t f;
...
void
UI_ParseMenu(const char *menuFile)
{
	int handle;
	pc_token_t token;
...
static const char *handicapValues[] = {"None", "95", "90", "85", "80", "75", "70", "65", "60", "55", "50", "45", "40", "35", "30", "25", "20", "15", "10", "5", nil};
#ifndef MISSIONPACK
...
static int
UI_TeamIndexFromName(const char *name)
{
	int i;
...
static void
UI_DrawTeamName(rectDef_t *rect, float scale, vec4_t color, qboolean blue, int textStyle)
{
	int i;
...
static void
UI_DrawTeamMember(rectDef_t *rect, float scale, vec4_t color, qboolean blue, int num, int textStyle)
{
	// 0 - None
	// 1 - Human
	// 2..NumCharacters - Bot
...
static void
UI_DrawTierMap(rectDef_t *rect, int index)
{
	int i;
	i = trap_Cvar_VariableValue("ui_currentTier");
	if(i < 0 || i >= uiInfo.tierCount)
...
static const char *
UI_EnglishMapName(const char *map)
{
	int i;
...
#ifndef MISSIONPACK
static const int
UI_AIIndexFromName(const char *name)
{
	int j;
	for(j = 0; j < uiInfo.aliasCount; j++)
		if(Q_stricmp(uiInfo.aliasList[j].name, name) == 0)
...
static void
UI_DrawAllMapsSelection(rectDef_t *rect, float scale, vec4_t color, int textStyle, qboolean net)
{
	int map = (net) ? ui_currentNetMap.integer : ui_currentMap.integer;
	if(map >= 0 && map < uiInfo.mapCount)
...
static int
UI_OwnerDrawWidth(int ownerDraw, float scale)
{
	int i, h, value;
	const char *text;
	const char *s = nil;
...
static void
UI_DrawServerRefreshDate(rectDef_t *rect, float scale, vec4_t color, int textStyle)
{
...
UI_GameType_HandleKey(int flags, float *special, int key, qboolean resetMap)
{
	if(key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER){
		...
		if(resetMap && oldCount != UI_MapCountByGameType(qtrue)){
			trap_Cvar_Set("ui_currentMap", "0");
			Menu_SetFeederSelection(nil, FEEDER_MAPS, 0, nil);
		}
		return qtrue;
	}
	return qfalse;
...
UI_NetGameType_HandleKey(int flags, float *special, int key)
{
	if(key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER){
		...
		Menu_SetFeederSelection(nil, FEEDER_ALLMAPS, 0, nil);
		return qtrue;
	}
	return qfalse;
}
...
static qboolean
UI_TeamMember_HandleKey(int flags, float *special, int key, qboolean blue, int num)
{
	if(key == K_MOUSE1 || key == K_MOUSE2 || key == K_ENTER || key == K_KP_ENTER){
		// 0 - None
		// 1 - Human
...
UI_SetNextMap(int actual, int index)
{
	...
	for(i = actual + 1; i < uiInfo.mapCount; i++)
		if(uiInfo.mapList[i].active){
			Menu_SetFeederSelection(nil, FEEDER_MAPS, index + 1, "skirmish");
			return qtrue;
		}
	return qfalse;
}
...
UI_StartSkirmish(qboolean next)
{
	...
	if(next){
		...
		if(UI_SetNextMap(actual, index)){
			...
			UI_GameType_HandleKey(0, nil, K_MOUSE1, qfalse);
			UI_MapCountByGameType(qtrue);
			Menu_SetFeederSelection(nil, FEEDER_MAPS, 0, "skirmish");
		}
	}
...
UI_RunMenuScript(char **args)
{
	...
	if(String_Parse(args, &name)){
		if(Q_stricmp(name, "StartServer") == 0){
			...
			Menu_SetFeederSelection(nil, FEEDER_MAPS, ui_mapIndex.integer, "skirmish");
			UI_GameType_HandleKey(0, nil, K_MOUSE1, qfalse);
			UI_GameType_HandleKey(0, nil, K_MOUSE2, qfalse);
			...
			UI_LoadArenas();
			UI_MapCountByGameType(qfalse);
			Menu_SetFeederSelection(nil, FEEDER_ALLMAPS, 0, "createserver");
			...
			UI_BuildServerStatus(qtrue);
			Menu_SetFeederSelection(nil, FEEDER_FINDPLAYER, 0, nil);
...
UI_hasSkinForBase(const char *base, const char *team)
{
	...
	if(trap_FS_FOpenFile(test, nil, FS_READ))
		return qtrue;
...
UI_HeadCountByTeam(void)
{
	...
	for(i = 0; i < uiInfo.characterCount; i++){
		...
		for(j = 0; j < TEAM_MEMBERS; j++){
			if(uiInfo.teamList[tIndex].teamMembers[j] != nil)
	...
	for(j = 0; j < TEAM_MEMBERS; j++)
		for(k = 0; k < uiInfo.aliasCount; k++){
			if(uiInfo.aliasList[k].name != nil)
...
*/
static void
UI_BuildServerDisplayList(int force)
{
...
//	qboolean startRefresh = qtrue; TTimo: unused
	...
	if(force){
		...
		Menu_SetFeederSelection(nil, FEEDER_SERVERS, 0, nil);
...
UI_GetServerStatusInfo(const char *serverAddress, serverStatusInfo_t *info)
{
	...
	if(!info){
		trap_LAN_ServerStatus(serverAddress, nil, 0);
		return qfalse;
	}
	...
	for(i = 0; i < MAX_SERVERSTATUSREQUESTS; i++){
		...
		if(!uiInfo.pendingServerStatus.server[i].valid ||
		   uiInfo.pendingServerStatus.server[i].startTime < uiInfo.uiDC.realTime - ui_serverStatusTimeOut.integer){
			...
			UI_GetServerStatusInfo(uiInfo.pendingServerStatus.server[i].adrstr, nil);
			// reuse pending slot
			uiInfo.pendingServerStatus.server[i].valid = qfalse;
...
UI_BuildServerStatus(qboolean force)
{
	...
	if(!force){
		...
		Menu_SetFeederSelection(nil, FEEDER_SERVERSTATUS, 0, nil);
		uiInfo.serverStatusInfo.numLines = 0;
		// reset all server status requests
		trap_LAN_ServerStatus(nil, nil, 0);
	}
	...
	if(UI_GetServerStatusInfo(uiInfo.serverStatusAddress, &uiInfo.serverStatusInfo)){
		uiInfo.nextServerStatusRefresh = 0;
		UI_GetServerStatusInfo(uiInfo.serverStatusAddress, nil);
	}else
...
static const char *
UI_SelectedMap(int index, int *actual)
{
	int i, c;
	c = 0;
	*actual = 0;
...
static const char *
UI_SelectedHead(int index, int *actual)
{
	int i, c;
	c = 0;
	*actual = 0;
...
static int
UI_GetIndexFromSelection(int actual)
{
	int i, c;
	c = 0;
	for(i = 0; i < uiInfo.mapCount; i++)
...
static const char *
UI_FeederItemText(float feederID, int index, int column, qhandle_t *handle)
{
...
static qhandle_t
UI_FeederItemImage(float feederID, int index)
{
	if(feederID == FEEDER_HEADS){
		int actual;
...
static void
UI_FeederSelection(float feederID, int index)
{
	static char info[MAX_STRING_CHARS];
	if(feederID == FEEDER_HEADS){
		int actual;
	...
	while(1){
		...
		if(token[0] == '{'){
			...
			for(i = 0; i < TEAM_MEMBERS; i++){
				uiInfo.teamList[uiInfo.teamCount].teamMembers[i] = nil;
...
// 2 - character parsing
static void
UI_ParseTeamInfo(const char *teamFile)
{
	char *token;
	char *p;
	char *buff = nil;
		...
		if(token[0] == '{'){
			...
			if(join){
				if(!String_Parse(p, &uiInfo.joinGameTypes[uiInfo.numJoinGameTypes].gameType) || !Int_Parse(p, &uiInfo.joinGameTypes[uiInfo.numJoinGameTypes].gtEnum))
					return qfalse;
			}else  if(!String_Parse(p, &uiInfo.gameTypes[uiInfo.numGameTypes].gameType) || !Int_Parse(p, &uiInfo.gameTypes[uiInfo.numGameTypes].gtEnum))
				return qfalse;
...
static void
UI_ParseGameInfo(const char *teamFile)
{
	char *token;
	char *p;
	char *buff = nil;
...
#ifndef MISSIONPACK
static int
UI_OwnerDraw_Width(int ownerDraw)
{
	return 0;
}
...
static int
UI_PlayCinematic(const char *name, float x, float y, float w, float h)
{
	return trap_CIN_PlayCinematic(name, x, y, w, h, (CIN_loop | CIN_silent));
}

static void
UI_StopCinematic(int handle)
{
	if(handle >= 0)
...
static void
UI_DrawCinematic(int handle, float x, float y, float w, float h)
{
...
static void
UI_RunCinematicFrame(int handle)
{
...
_UI_Init(qboolean inGameLoad)
{
	...
	trap_Cvar_Register(nil, "debug_protocol", "", 0);
...
_UI_MouseEvent(int dx, int dy)
{
	...
	if(Menu_Count() > 0)
		...
		Display_MouseMove(nil, uiInfo.uiDC.cursorx, uiInfo.uiDC.cursory);

}

void
UI_LoadNonIngame(void)
{
...
_UI_SetActiveMenu(uiMenuCommand_t menu)
{
	...
	if(Menu_Count() > 0){
		...
		case UIMENU_MAIN:
			...
			//trap_S_StartBackgroundTrack("sound/misc/menu_background.wav", nil);
			if(uiInfo.inGameLoad)
				UI_LoadNonIngame();
			Menus_CloseAll();
			Menus_ActivateByName("main");
		...
		case UIMENU_NEED_CD:
			...
			//UI_ConfirmMenu( "Insert the CD", nil, NeedCDAction );
			return;
		case UIMENU_BAD_CD_KEY:
			// no cd check in TA
			//trap_Key_SetCatcher( KEYCATCH_UI );
			//Menus_ActivateByName("badcd");
			//UI_ConfirmMenu( "Bad CD Key", nil, NeedCDKeyAction );
			return;
...
void
Text_PaintCenter_AutoWrapped(float x, float y, float xmax, float ystep, float scale, vec4_t color, const char *str, float adjust)
{
	int width;
	char *s1, *s2, *s3;
	char c_bcp;
...
static void
UI_DisplayDownloadInfo(const char *downloadName, float centerPoint, float yStart, float scale)
{
	...
	if(ui_netSource.integer != UIAS_FAVORITES){
		if(ui_netSource.integer == UIAS_LOCAL){
			if(!trap_LAN_GetServerCount(AS_LOCAL))
				wait = qtrue;
		}else  if(trap_LAN_GetServerCount(AS_GLOBAL) < 0)
			wait = qtrue;
