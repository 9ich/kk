...
static void *captureData = nil;
static itemDef_t *itemCapture = nil;	// item that has the mouse captured ( if any )

displayContextDef_t *DC = nil;

static qboolean g_waitingForKey = qfalse;
static qboolean g_editingField = qfalse;

static itemDef_t *g_bindItem = nil;
static itemDef_t *g_editItem = nil;
...
const char *
String_Alloc(const char *p)
{
	int len;
	uint hash;
	stringDef_t *str, *last;
	static const char *staticnil = "";
...
void
Fade(int *flags, float *f, float clamp, int *nextTime, int offsetTime, qboolean bFlags, float fadeAmount)
{
...
void
Item_SetScreenCoords(itemDef_t *item, float x, float y)
{
	if(item == nil)
		return;
...
void
Item_UpdatePosition(itemDef_t *item)
{
	float x, y;
	menuDef_t *menu;
...
// menus
void
Menu_UpdatePosition(menuDef_t *menu)
{
	int i;
	float x, y;
...
itemDef_t *
Menu_ClearFocus(menuDef_t *menu)
{
	int i;
	itemDef_t *ret = nil;
...
qboolean
Rect_ContainsPoint(rectDef_t *rect, float x, float y)
{
	if(rect)
...
int
Menu_ItemsMatchingGroup(menuDef_t *menu, const char *name)
{
	int i;
	int count = 0;
	for(i = 0; i < menu->itemCount; i++)
...
itemDef_t *
Menu_GetMatchingItemByNumber(menuDef_t *menu, int index, const char *name)
{
	int i;
	int count = 0;
	for(i = 0; i < menu->itemCount; i++)
...
void
Script_SetColor(itemDef_t *item, char **args)
{
	const char *name;
	int i;
	float f;
...
void
Script_SetAsset(itemDef_t *item, char **args)
{
	const char *name;
...
void
Script_SetBackground(itemDef_t *item, char **args)
{
	const char *name;
	// expecting name to set asset to
	if(String_Parse(args, &name))
...
itemDef_t *
Menu_FindItemByName(menuDef_t *menu, const char *p)
{
	int i;
	if(menu == nil || p == nil)
		return nil;
...
void
Script_SetTeamColor(itemDef_t *item, char **args)
{
	if(DC->getTeamColor){
		int i;
		vec4_t color;
...
void
Script_SetItemColor(itemDef_t *item, char **args)
{
	const char *itemname;
	const char *name;
	vec4_t color;
...
void
Menu_ShowItemByName(menuDef_t *menu, const char *p, qboolean bShow)
{
	itemDef_t *item;
	int i;
...
void
Menu_FadeItemByName(menuDef_t *menu, const char *p, qboolean fadeOut)
{
	itemDef_t *item;
	int i;
	int count = Menu_ItemsMatchingGroup(menu, p);
...
void
Menus_OpenByName(const char *p)
{
	Menus_ActivateByName(p);
}

static void
Menu_RunCloseScript(menuDef_t *menu)
{
...
void
Menus_CloseByName(const char *p)
{
	menuDef_t *menu = Menus_FindByName(p);
	if(menu != nil){
...
void
Script_Show(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Script_Hide(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Script_FadeIn(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Script_FadeOut(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Script_Open(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Script_ConditionalOpen(itemDef_t *item, char **args)
{
	const char *cvar;
	const char *name1;
	const char *name2;
...
void
Script_Close(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Menu_TransitionItemByName(menuDef_t *menu, const char *p, rectDef_t rectFrom, rectDef_t rectTo, int time, float amt)
{
	itemDef_t *item;
	int i;
	int count = Menu_ItemsMatchingGroup(menu, p);
...
void
Script_Transition(itemDef_t *item, char **args)
{
	const char *name;
...
void
Menu_OrbitItemByName(menuDef_t *menu, const char *p, float x, float y, float cx, float cy, int time)
{
	itemDef_t *item;
	int i;
	int count = Menu_ItemsMatchingGroup(menu, p);
...
void
Script_Orbit(itemDef_t *item, char **args)
{
	const char *name;
	float cx, cy, x, y;
	int time;
...
void
Script_SetFocus(itemDef_t *item, char **args)
{
	const char *name;
	itemDef_t *focusItem;
...
void
Script_SetPlayerModel(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Script_SetPlayerHead(itemDef_t *item, char **args)
{
	const char *name;
	if(String_Parse(args, &name))
...
void
Script_SetCvar(itemDef_t *item, char **args)
{
	const char *cvar, *val;
...
void
Script_Exec(itemDef_t *item, char **args)
{
	const char *val;
...
void
Script_Play(itemDef_t *item, char **args)
{
	const char *val;
	if(String_Parse(args, &val))
...
void
Script_playLooped(itemDef_t *item, char **args)
{
	const char *val;
	if(String_Parse(args, &val)){
...
void
Item_RunScript(itemDef_t *item, const char *s)
{
	char script[1024], *p;
	int i;
	qboolean bRan;
	...
	if(item && s && s[0]){
		...
		while(1){
			const char *command;
			// expect command then arguments, ; ends command, nil ends script
...
qboolean
Item_EnableShowViaCvar(itemDef_t *item, int flag)
{
	char script[1024], *p;
	memset(script, 0, sizeof(script));
	if(item && item->enableCvar && *item->enableCvar && item->cvarTest && *item->cvarTest){
		...
		while(1){
			const char *val;
			// expect value then ; or nil, nil ends list
...
qboolean
Item_SetFocus(itemDef_t *item, float x, float y)
{
	int i;
	itemDef_t *oldFocus;
	...
	if(item == nil || item->window.flags & WINDOW_DECORATION || item->window.flags & WINDOW_HASFOCUS || !(item->window.flags & WINDOW_VISIBLE))
		return qfalse;
...
int
Item_ListBox_MaxScroll(itemDef_t *item)
{
...
int
Item_ListBox_ThumbPosition(itemDef_t *item)
{
	float max, pos, size;
	listBoxDef_t *listPtr = (listBoxDef_t*)item->typeData;
...
int
Item_ListBox_ThumbDrawPosition(itemDef_t *item)
{
	int min, max;
...
float
Item_Slider_ThumbPosition(itemDef_t *item)
{
	float value, range, x;
...
int
Item_Slider_OverSlider(itemDef_t *item, float x, float y)
{
	rectDef_t r;
...
int
Item_ListBox_OverLB(itemDef_t *item, float x, float y)
{
	rectDef_t r;
	int thumbstart;
...
void
Item_ListBox_MouseEnter(itemDef_t *item, float x, float y)
{
	rectDef_t r;
	listBoxDef_t *listPtr = (listBoxDef_t*)item->typeData;
...
void
Item_MouseEnter(itemDef_t *item, float x, float y)
{
	rectDef_t r;
	if(item){
		r = item->textRect;
...
void
Item_MouseLeave(itemDef_t *item)
{
	if(item){
		if(item->window.flags & WINDOW_MOUSEOVERTEXT){
			Item_RunScript(item, item->mouseExitText);
...
itemDef_t *
Menu_HitTest(menuDef_t *menu, float x, float y)
{
	int i;
	for(i = 0; i < menu->itemCount; i++)
...
void
Item_SetMouseOver(itemDef_t *item, qboolean focus)
{
	if(item){
		if(focus)
			item->window.flags |= WINDOW_MOUSEOVER;
...
qboolean
Item_OwnerDraw_HandleKey(itemDef_t *item, int key)
{
...
qboolean
Item_ListBox_HandleKey(itemDef_t *item, int key, qboolean down, qboolean force)
{
	listBoxDef_t *listPtr = (listBoxDef_t*)item->typeData;
	int count = DC->feederCount(item->special);
	int max, viewmax;
...
qboolean
Item_YesNo_HandleKey(itemDef_t *item, int key)
{
...
int
Item_Multi_CountSettings(itemDef_t *item)
{
...
int
Item_Multi_FindCvarByValue(itemDef_t *item)
{
	char buff[1024];
	float value = 0;
	int i;
...
const char *
Item_Multi_Setting(itemDef_t *item)
{
	char buff[1024];
	float value = 0;
	int i;
...
qboolean
Item_Multi_HandleKey(itemDef_t *item, int key)
{
	multiDef_t *multiPtr = (multiDef_t*)item->typeData;
	if(multiPtr){
		if(Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) && item->window.flags & WINDOW_HASFOCUS && item->cvar)
...
qboolean
Item_TextField_HandleKey(itemDef_t *item, int key)
{
	char buff[1024];
	int len;
	itemDef_t *newItem = nil;
		...
		if(key & K_CHAR_FLAG){
			...
			if(!DC->getOverstrikeMode()){
				if((len == MAX_EDITFIELD - 1) || (editPtr->maxChars && len >= editPtr->maxChars))
					return qtrue;
				memmove(&buff[item->cursorPos + 1], &buff[item->cursorPos], len + 1 - item->cursorPos);
			}else  if(editPtr->maxChars && item->cursorPos >= editPtr->maxChars)
				return qtrue;
...
static void
Scroll_ListBox_AutoFunc(void *p)
{
...
static void
Scroll_ListBox_ThumbFunc(void *p)
{
	scrollInfo_t *si = (scrollInfo_t*)p;
	rectDef_t r;
	int pos, max;
...
static void
Scroll_Slider_ThumbFunc(void *p)
{
...
void
Item_StartCapture(itemDef_t *item, int key)
{
	int flags;
	switch(item->type){
...
void
Item_StopCapture(itemDef_t *item)
{
}

qboolean
Item_Slider_HandleKey(itemDef_t *item, int key, qboolean down)
{
...
qboolean
Item_HandleKey(itemDef_t *item, int key, qboolean down)
{
	if(itemCapture){
		...
		captureFunc = 0;
		captureData = nil;
	}else  if(down && (key == K_MOUSE1 || key == K_MOUSE2 || key == K_MOUSE3))
...
itemDef_t *
Menu_SetPrevCursorItem(menuDef_t *menu)
{
...
itemDef_t *
Menu_SetNextCursorItem(menuDef_t *menu)
{
	qboolean wrapped = qfalse;
	int oldCursor = menu->cursorItem;
...
static void
Window_CloseCinematic(windowDef_t *window)
{
...
static void
Menu_CloseCinematics(menuDef_t *menu)
{
	if(menu){
		int i;
...
void
Menus_HandleOOBClick(menuDef_t *menu, int key, qboolean down)
{
	if(menu){
		int i;
...
static rectDef_t *
Item_CorrectedTextRect(itemDef_t *item)
{
...
void
Menu_HandleKey(menuDef_t *menu, int key, qboolean down)
{
	int i;
	itemDef_t *item = nil;
	...
	if(g_editingField && down){
		if(!Item_TextField_HandleKey(g_editItem, key)){
			...
			g_editingField = qfalse;
			g_editItem = nil;
			Display_MouseMove(nil, DC->cursorx, DC->cursory);
...
void
ToWindowCoords(float *x, float *y, windowDef_t *window)
{
	if(window->border != 0){
		*x += window->borderSize;
		*y += window->borderSize;
...
void
Rect_ToWindowCoords(rectDef_t *rect, windowDef_t *window)
{
	ToWindowCoords(&rect->x, &rect->y, window);
}

void
Item_SetTextExtents(itemDef_t *item, int *width, int *height, const char *text)
{
...
void
Item_Text_AutoWrapped_Paint(itemDef_t *item)
{
	char text[1024];
...
void
Item_Text_Wrapped_Paint(itemDef_t *item)
{
	char text[1024];
...
void
Item_Text_Paint(itemDef_t *item)
{
	char text[1024];
	const char *textPtr;
	int height, width;
...
int
BindingIDFromName(const char *name)
{
	int i;
	for(i = 0; i < g_bindCount; i++)
...
void
BindingFromName(const char *cvar)
{
	int i, b1, b2;
...
qboolean
Display_KeyBindPending(void)
{
	return g_waitingForKey;
}

qboolean
Item_Bind_HandleKey(itemDef_t *item, int key, qboolean down)
{
	int id;
	int i;
	...
	if(Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) && !g_waitingForKey){
		...
		if(!g_waitingForKey || g_bindItem == nil)
			return qtrue;
...
void
AdjustFrom640(float *x, float *y, float *w, float *h)
{
...
void
Item_Model_Paint(itemDef_t *item)
{
	float x, y, w, h;
	refdef_t refdef;
	refEntity_t ent;
...
void
Item_Image_Paint(itemDef_t *item)
{
	if(item == nil)
		return;
...
void
Item_ListBox_Paint(itemDef_t *item)
{
...
void
Item_OwnerDraw_Paint(itemDef_t *item)
{
	if(item == nil)
		return;
...
itemDef_t *
Menu_GetFocusedItem(menuDef_t *menu)
{
	int i;
	if(menu)
		for(i = 0; i < menu->itemCount; i++)
...
void
Menu_ScrollFeeder(menuDef_t *menu, int feeder, qboolean down)
{
	if(menu){
		int i;
		for(i = 0; i < menu->itemCount; i++)
...
void
Menu_SetFeederSelection(menuDef_t *menu, int feeder, int index, const char *name)
{
	if(menu == nil){
		if(name == nil)
...
qboolean
Menus_AnyFullScreenVisible(void)
{
	int i;
	for(i = 0; i < menuCount; i++)
...
menuDef_t *
Menus_ActivateByName(const char *p)
{
	int i;
	menuDef_t *m = nil;
	...
	for(i = 0; i < menuCount; i++){
		if(Q_stricmp(Menus[i].window.name, p) == 0){
			m = &Menus[i];
			Menus_Activate(m);
			if(openMenuCount < MAX_OPEN_MENUS && focus != nil)
...
void
Menu_HandleMouseMove(menuDef_t *menu, float x, float y)
{
	int i, pass;
...
void
Menu_Paint(menuDef_t *menu, qboolean forcePaint)
{
	int i;
...
void
KeywordHash_Add(keywordHash_t *table[], keywordHash_t *key)
{
	int hash;
...
keywordHash_t *
KeywordHash_Find(keywordHash_t *table[], char *keyword)
{
	keywordHash_t *key;
	int hash;
...
// name <string>
qboolean
ItemParse_name(itemDef_t *item, int handle)
{
...
// name <string>
qboolean
ItemParse_focusSound(itemDef_t *item, int handle)
{
	const char *temp;
...
// text <string>
qboolean
ItemParse_text(itemDef_t *item, int handle)
{
...
// group <string>
qboolean
ItemParse_group(itemDef_t *item, int handle)
{
...
// asset_model <string>
qboolean
ItemParse_asset_model(itemDef_t *item, int handle)
{
	const char *temp;
	modelDef_t *modelPtr;
...
// asset_shader <string>
qboolean
ItemParse_asset_shader(itemDef_t *item, int handle)
{
	const char *temp;
...
qboolean
ItemParse_model_origin(itemDef_t *item, int handle)
{
	modelDef_t *modelPtr;
	Item_ValidateTypeData(item);
	modelPtr = (modelDef_t*)item->typeData;
...
// model_fovx <number>
qboolean
ItemParse_model_fovx(itemDef_t *item, int handle)
{
	modelDef_t *modelPtr;
	Item_ValidateTypeData(item);
	modelPtr = (modelDef_t*)item->typeData;
...
// model_fovy <number>
qboolean
ItemParse_model_fovy(itemDef_t *item, int handle)
{
	modelDef_t *modelPtr;
	Item_ValidateTypeData(item);
	modelPtr = (modelDef_t*)item->typeData;
...
qboolean
ItemParse_model_rotation(itemDef_t *item, int handle)
{
	modelDef_t *modelPtr;
	Item_ValidateTypeData(item);
	modelPtr = (modelDef_t*)item->typeData;
...
// model_angle <integer>
qboolean
ItemParse_model_angle(itemDef_t *item, int handle)
{
	modelDef_t *modelPtr;
	Item_ValidateTypeData(item);
	modelPtr = (modelDef_t*)item->typeData;
...
// rect <rectangle>
qboolean
ItemParse_rect(itemDef_t *item, int handle)
{
...
// style <integer>
qboolean
ItemParse_style(itemDef_t *item, int handle)
{
...
// decoration
qboolean
ItemParse_decoration(itemDef_t *item, int handle)
{
...
// notselectable
qboolean
ItemParse_notselectable(itemDef_t *item, int handle)
{
...
// manually wrapped
qboolean
ItemParse_wrapped(itemDef_t *item, int handle)
{
...
// auto wrapped
qboolean
ItemParse_autowrapped(itemDef_t *item, int handle)
{
...
// horizontalscroll
qboolean
ItemParse_horizontalscroll(itemDef_t *item, int handle)
{
...
// type <integer>
qboolean
ItemParse_type(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_elementwidth(itemDef_t *item, int handle)
{
	listBoxDef_t *listPtr;
...
qboolean
ItemParse_elementheight(itemDef_t *item, int handle)
{
	listBoxDef_t *listPtr;
...
// feeder <float>
qboolean
ItemParse_feeder(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_elementtype(itemDef_t *item, int handle)
{
	listBoxDef_t *listPtr;
...
qboolean
ItemParse_columns(itemDef_t *item, int handle)
{
	int num, i;
	listBoxDef_t *listPtr;
...
qboolean
ItemParse_border(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_bordersize(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_visible(itemDef_t *item, int handle)
{
	int i;
...
qboolean
ItemParse_ownerdraw(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_align(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_textalign(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_textalignx(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_textaligny(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_textscale(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_textstyle(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_backcolor(itemDef_t *item, int handle)
{
	int i;
	float f;
...
qboolean
ItemParse_forecolor(itemDef_t *item, int handle)
{
	int i;
	float f;
...
qboolean
ItemParse_bordercolor(itemDef_t *item, int handle)
{
	int i;
	float f;
...
qboolean
ItemParse_outlinecolor(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_background(itemDef_t *item, int handle)
{
	const char *temp;
...
qboolean
ItemParse_cinematic(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_doubleClick(itemDef_t *item, int handle)
{
	listBoxDef_t *listPtr;
...
qboolean
ItemParse_onFocus(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_leaveFocus(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_mouseEnter(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_mouseExit(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_mouseEnterText(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_mouseExitText(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_action(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_special(itemDef_t *item, int handle)
{
	if(!PC_Float_Parse(handle, &item->special))
		return qfalse;
	return qtrue;
...
qboolean
ItemParse_cvarTest(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_cvar(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_maxChars(itemDef_t *item, int handle)
{
	editFieldDef_t *editPtr;
	int maxChars;
...
qboolean
ItemParse_maxPaintChars(itemDef_t *item, int handle)
{
	editFieldDef_t *editPtr;
	int maxChars;
...
qboolean
ItemParse_cvarFloat(itemDef_t *item, int handle)
{
	editFieldDef_t *editPtr;
...
qboolean
ItemParse_cvarStrList(itemDef_t *item, int handle)
{
	pc_token_t token;
	multiDef_t *multiPtr;
	int pass;
...
qboolean
ItemParse_cvarFloatList(itemDef_t *item, int handle)
{
	pc_token_t token;
	multiDef_t *multiPtr;
...
qboolean
ItemParse_addColorRange(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_ownerdrawFlag(itemDef_t *item, int handle)
{
	int i;
	if(!PC_Int_Parse(handle, &i))
		return qfalse;
...
qboolean
ItemParse_enableCvar(itemDef_t *item, int handle)
{
...
qboolean
ItemParse_disableCvar(itemDef_t *item, int handle)
{
	if(PC_Script_Parse(handle, &item->enableCvar)){
...
qboolean
ItemParse_showCvar(itemDef_t *item, int handle)
{
	if(PC_Script_Parse(handle, &item->enableCvar)){
...
qboolean
ItemParse_hideCvar(itemDef_t *item, int handle)
{
	if(PC_Script_Parse(handle, &item->enableCvar)){
...
keywordHash_t itemParseKeywords[] = {
	{"name", ItemParse_name, nil},
	{"text", ItemParse_text, nil},
	{"group", ItemParse_group, nil},
	{"asset_model", ItemParse_asset_model, nil},
	{"asset_shader", ItemParse_asset_shader, nil},
	{"model_origin", ItemParse_model_origin, nil},
	{"model_fovx", ItemParse_model_fovx, nil},
	{"model_fovy", ItemParse_model_fovy, nil},
	{"model_rotation", ItemParse_model_rotation, nil},
	{"model_angle", ItemParse_model_angle, nil},
	{"rect", ItemParse_rect, nil},
	{"style", ItemParse_style, nil},
	{"decoration", ItemParse_decoration, nil},
	{"notselectable", ItemParse_notselectable, nil},
	{"wrapped", ItemParse_wrapped, nil},
	{"autowrapped", ItemParse_autowrapped, nil},
	{"horizontalscroll", ItemParse_horizontalscroll, nil},
	{"type", ItemParse_type, nil},
	{"elementwidth", ItemParse_elementwidth, nil},
	{"elementheight", ItemParse_elementheight, nil},
	{"feeder", ItemParse_feeder, nil},
	{"elementtype", ItemParse_elementtype, nil},
	{"columns", ItemParse_columns, nil},
	{"border", ItemParse_border, nil},
	{"bordersize", ItemParse_bordersize, nil},
	{"visible", ItemParse_visible, nil},
	{"ownerdraw", ItemParse_ownerdraw, nil},
	{"align", ItemParse_align, nil},
	{"textalign", ItemParse_textalign, nil},
	{"textalignx", ItemParse_textalignx, nil},
	{"textaligny", ItemParse_textaligny, nil},
	{"textscale", ItemParse_textscale, nil},
	{"textstyle", ItemParse_textstyle, nil},
	{"backcolor", ItemParse_backcolor, nil},
	{"forecolor", ItemParse_forecolor, nil},
	{"bordercolor", ItemParse_bordercolor, nil},
	{"outlinecolor", ItemParse_outlinecolor, nil},
	{"background", ItemParse_background, nil},
	{"onFocus", ItemParse_onFocus, nil},
	{"leaveFocus", ItemParse_leaveFocus, nil},
	{"mouseEnter", ItemParse_mouseEnter, nil},
	{"mouseExit", ItemParse_mouseExit, nil},
	{"mouseEnterText", ItemParse_mouseEnterText, nil},
	{"mouseExitText", ItemParse_mouseExitText, nil},
	{"action", ItemParse_action, nil},
	{"special", ItemParse_special, nil},
	{"cvar", ItemParse_cvar, nil},
	{"maxChars", ItemParse_maxChars, nil},
	{"maxPaintChars", ItemParse_maxPaintChars, nil},
	{"focusSound", ItemParse_focusSound, nil},
	{"cvarFloat", ItemParse_cvarFloat, nil},
	{"cvarStrList", ItemParse_cvarStrList, nil},
	{"cvarFloatList", ItemParse_cvarFloatList, nil},
	{"addColorRange", ItemParse_addColorRange, nil},
	{"ownerdrawFlag", ItemParse_ownerdrawFlag, nil},
	{"enableCvar", ItemParse_enableCvar, nil},
	{"cvarTest", ItemParse_cvarTest, nil},
	{"disableCvar", ItemParse_disableCvar, nil},
	{"showCvar", ItemParse_showCvar, nil},
	{"hideCvar", ItemParse_hideCvar, nil},
	{"cinematic", ItemParse_cinematic, nil},
	{"doubleclick", ItemParse_doubleClick, nil},
	{nil, 0, nil}
};
...
*/
static void
Item_ApplyHacks(itemDef_t *item)
{
...
void
Item_InitControls(itemDef_t *item)
{
	if(item == nil)
		return;
	if(item->type == ITEM_TYPE_LISTBOX){
...
qboolean
MenuParse_font(itemDef_t *item, int handle)
{
...
qboolean
MenuParse_name(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_fullscreen(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
	union
	{
...
qboolean
MenuParse_rect(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_style(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_visible(itemDef_t *item, int handle)
{
	int i;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_onOpen(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_onClose(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_onESC(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_border(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_borderSize(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_backcolor(itemDef_t *item, int handle)
{
	int i;
	float f;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_forecolor(itemDef_t *item, int handle)
{
	int i;
	float f;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_bordercolor(itemDef_t *item, int handle)
{
	int i;
	float f;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_focuscolor(itemDef_t *item, int handle)
{
	int i;
	float f;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_disablecolor(itemDef_t *item, int handle)
{
	int i;
	float f;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_outlinecolor(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_background(itemDef_t *item, int handle)
{
	const char *buff;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_cinematic(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_ownerdrawFlag(itemDef_t *item, int handle)
{
	int i;
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_ownerdraw(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
// decoration
qboolean
MenuParse_popup(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_outOfBounds(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_soundLoop(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_fadeClamp(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_fadeAmount(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_fadeCycle(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
qboolean
MenuParse_itemDef(itemDef_t *item, int handle)
{
	menuDef_t *menu = (menuDef_t*)item;
...
keywordHash_t menuParseKeywords[] = {
	{"font", MenuParse_font, nil},
	{"name", MenuParse_name, nil},
	{"fullscreen", MenuParse_fullscreen, nil},
	{"rect", MenuParse_rect, nil},
	{"style", MenuParse_style, nil},
	{"visible", MenuParse_visible, nil},
	{"onOpen", MenuParse_onOpen, nil},
	{"onClose", MenuParse_onClose, nil},
	{"onESC", MenuParse_onESC, nil},
	{"border", MenuParse_border, nil},
	{"borderSize", MenuParse_borderSize, nil},
	{"backcolor", MenuParse_backcolor, nil},
	{"forecolor", MenuParse_forecolor, nil},
	{"bordercolor", MenuParse_bordercolor, nil},
	{"focuscolor", MenuParse_focuscolor, nil},
	{"disablecolor", MenuParse_disablecolor, nil},
	{"outlinecolor", MenuParse_outlinecolor, nil},
	{"background", MenuParse_background, nil},
	...
	{"fadeClamp", MenuParse_fadeClamp, nil},
	{"fadeCycle", MenuParse_fadeCycle, nil},
	{"fadeAmount", MenuParse_fadeAmount, nil},
	{nil, 0, nil}
};
...
void *
Display_CaptureItem(int x, int y)
{
	int i;
...
// FIXME:
qboolean
Display_MouseMove(void *p, int x, int y)
{
	int i;
	menuDef_t *menu = p;
...
int
Display_CursorType(int x, int y)
{
	int i;
	for(i = 0; i < menuCount; i++){
		rectDef_t r2;
...
void
Display_HandleKey(int key, qboolean down, int x, int y)
{
...
static void
Window_CacheContents(windowDef_t *window)
{
	if(window)
...
static void
Item_CacheContents(itemDef_t *item)
{
	if(item)
...
static void
Menu_CacheContents(menuDef_t *menu)
{
	if(menu){
		int i;
...
static qboolean
Menu_OverActiveItem(menuDef_t *menu, float x, float y)
{
