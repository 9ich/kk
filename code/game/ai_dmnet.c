...
BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal)
{
	...
	if(bs->ltgtype == LTG_TEAMACCOMPANY && !retreat){
		...
		if(bs->teamgoal_time < FloatTime()){
			BotAI_BotInitialChat(bs, "accompany_stop", EasyClientName(bs->teammate, netname, sizeof(netname)), nil);
		...
		if(BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->teammate)){
			...
			bs->teammatevisible_time = FloatTime();
			vecsub(entinfo.origin, bs->origin, dir);
			if(veclensq(dir) < Square(bs->formation_dist)){
						   ...
						   botinfo.origin[1] + botinfo.mins[1] < entinfo.origin[1] + entinfo.maxs[1] + 4)
							if(botinfo.origin[2] + botinfo.maxs[2] > entinfo.origin[2] + entinfo.mins[2] - 4 &&
							   botinfo.origin[2] + botinfo.mins[2] < entinfo.origin[2] + entinfo.maxs[2] + 4){
								...
								anglevecs(entinfo.angles, dir, nil, nil);
								dir[2] = 0;
								vecnorm(dir);
								//vecsub(entinfo.origin, entinfo.lastvisorigin, dir);
								vecsub(bs->origin, entinfo.origin, dir2);
								vecnorm(dir2);
								if(vecdot(dir, dir2) > 0.7){
									// back up
				...
				if(bs->arrive_time < FloatTime() - 2){
					...
					if(!bs->arrive_time){
						...
						BotAI_BotInitialChat(bs, "accompany_arrive", EasyClientName(bs->teammate, netname, sizeof(netname)), nil);
						trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
		...
		if(bs->teammatevisible_time < FloatTime() - 60){
			BotAI_BotInitialChat(bs, "accompany_cannotfind", EasyClientName(bs->teammate, netname, sizeof(netname)), nil);
			trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
			bs->ltgtype = 0;
	...
	if(bs->ltgtype == LTG_DEFENDKEYAREA && !retreat &&
	   bs->defendaway_time < FloatTime()){
		...
		if(bs->teamgoal_time < FloatTime()){
			trap_BotGoalName(bs->teamgoal.number, buf, sizeof(buf));
			BotAI_BotInitialChat(bs, "defend_stop", buf, nil);
			trap_BotEnterChat(bs->cs, 0, CHAT_TEAM);
			bs->ltgtype = 0;
		}
		//if very close... go away for some time
		vecsub(goal->origin, bs->origin, dir);
		if(veclensq(dir) < Square(70)){
			trap_BotResetAvoidReach(bs->ms);
	...
	if(bs->ltgtype == LTG_KILL && !retreat){
		...
		if(bs->teammessage_time && bs->teammessage_time < FloatTime()){
			...
			BotAI_BotInitialChat(bs, "kill_start", buf, nil);
			trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
			bs->teammessage_time = 0;
		}
		if(bs->lastkilledplayer == bs->teamgoal.entitynum){
			EasyClientName(bs->teamgoal.entitynum, buf, sizeof(buf));
			BotAI_BotInitialChat(bs, "kill_done", buf, nil);
			trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
	...
	if(bs->ltgtype == LTG_GETITEM && !retreat){
		...
		if(trap_BotItemGoalInVisButNotVisible(bs->entitynum, bs->eye, bs->viewangles, goal)){
			trap_BotGoalName(bs->teamgoal.number, buf, sizeof(buf));
			BotAI_BotInitialChat(bs, "getitem_notthere", buf, nil);
			trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
			bs->ltgtype = 0;
		}else if(BotReachedGoal(bs, goal)){
			trap_BotGoalName(bs->teamgoal.number, buf, sizeof(buf));
			BotAI_BotInitialChat(bs, "getitem_gotit", buf, nil);
			trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
			bs->ltgtype = 0;
		}
		return qtrue;
	...
	if((bs->ltgtype == LTG_CAMP || bs->ltgtype == LTG_CAMPORDER) && !retreat){
		...
		if(bs->teamgoal_time < FloatTime()){
			if(bs->ltgtype == LTG_CAMPORDER){
				BotAI_BotInitialChat(bs, "camp_stop", nil);
				trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
			}
			bs->ltgtype = 0;
		}
	...
	if(bs->ltgtype == LTG_PATROL && !retreat){
		...
		if(bs->teamgoal_time < FloatTime()){
			BotAI_BotInitialChat(bs, "patrol_stop", nil);
			trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
			bs->ltgtype = 0;
		}
		if(!bs->curpatrolpoint){
	...
	if(gametype == GT_CTF){
		...
		if(bs->ltgtype == LTG_ATTACKENEMYBASE &&
		   bs->attackaway_time < FloatTime()){
			...
			vecsub(bs->origin, goal->origin, dir);
			if(veclensq(dir) < Square(60))
				bs->attackaway_time = FloatTime() + 3 + 5 * random();
			//quit rushing after 2 minutes
			if(bs->teamgoal_time < FloatTime())
...
BotLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal)
{
	...
	if(bs->lead_time > 0 && !retreat){
		if(bs->lead_time < FloatTime()){
			BotAI_BotInitialChat(bs, "lead_stop", EasyClientName(bs->lead_teammate, teammate, sizeof(teammate)), nil);
			trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
			bs->lead_time = 0;
			return BotGetLongTermGoal(bs, tfl, retreat, goal);
		}
		if(bs->leadmessage_time < 0 && -bs->leadmessage_time < FloatTime()){
			BotAI_BotInitialChat(bs, "followme", EasyClientName(bs->lead_teammate, teammate, sizeof(teammate)), nil);
			trap_BotEnterChat(bs->cs, bs->teammate, CHAT_TELL);
		...
		vecsub(bs->origin, bs->lead_teamgoal.origin, dir);
	...
	if(bs->kamikazebody)
		...
		if(!(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON))){
			...
			if(moveresult->weapon){
				...
				if(bs->cur_ps.weapon[0] == moveresult->weapon)
					// if the bot is pretty close with its aim
					if(InFieldOfVision(bs->viewangles, 20, moveresult->ideal_viewangles)){
						BotAI_Trace(&bsptrace, bs->eye, nil, nil, target, bs->entitynum, MASK_SHOT);
	...
	if(bs->blockedbyavoidspot_time > FloatTime() &&
	   !(moveresult->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_MOVEMENTWEAPON))){
		...
		for(i = 0; i < bs->numproxmines; i++){
			...
			vecsub(state.pos.trBase, bs->origin, dir);
			dist = veclen(dir);
			if(dist < bestdist){
				bestdist = dist;
				bestmine = i;
...
AINode_Seek_ActivateEntity(bot_state_t *bs)
{
	...
	if(bs->activatestack->shoot){
		BotAI_Trace(&bsptrace, bs->eye, nil, nil, bs->activatestack->target, bs->entitynum, MASK_SHOT);
		// if the shootable entity is visible from the current position
		if(bsptrace.fraction >= 1.0 || bsptrace.ent == goal->entitynum){
			targetvisible = qtrue;
			// if holding the right weapon
			if(bs->cur_ps.weapon[0] == bs->activatestack->weapon){
				vecsub(bs->activatestack->target, bs->eye, dir);
	...
	if(targetvisible){
		...
		if(!veccmp(bs->activatestack->origin, entinfo.origin)){
#ifdef DEBUG
...
AINode_Seek_LTG(bot_state_t *bs)
{
	...
	//if waiting for something
		...
		//FIXME: look at cluster portals?
		else if(veclensq(moveresult.movedir))
			vectoangles(moveresult.movedir, bs->ideal_viewangles);
		else if(random() < bs->thinktime * 0.8){
			BotRoamGoal(bs, target);
			vecsub(target, bs->origin, dir);
...
AINode_Battle_Fight(bot_state_t *bs)
{
	...
	if(bs->cur_ps.persistant[PERS_DMG] > bs->lasthitcount)
