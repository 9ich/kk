...
SelectNearestDeathmatchSpawnPoint(vec3_t from)
{
	...
	while((spot = G_Find(spot, FOFS(classname), "info_player_deathmatch")) != nil){
...
SelectRandomDeathmatchSpawnPoint(qboolean isbot)
{
	...
	while((spot = G_Find(spot, FOFS(classname), "info_player_deathmatch")) != nil && count < MAX_SPAWN_POINTS){
	...
	if(!count)	// no spots that won't telefrag
		return G_Find(nil, FOFS(classname), "info_player_deathmatch");
...
SelectRandomFurthestSpawnPoint(vec3_t avoidPoint, vec3_t origin, vec3_t angles, qboolean isbot)
{
	...
	if(!numSpots){
		spot = G_Find(nil, FOFS(classname), "info_player_deathmatch");
