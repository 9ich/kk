...
void
trap_Cvar_Register(vmCvar_t *vmCvar, const char *varName, const char *defaultValue, int flags)
{
...
void
trap_CM_TransformedBoxTrace(trace_t *results, const vec3_t start, const vec3_t end,
			    const vec3_t mins, const vec3_t maxs,
			    clipHandle_t model, int brushmask,
...
void
trap_CM_TransformedCapsuleTrace(trace_t *results, const vec3_t start, const vec3_t end,
				const vec3_t mins, const vec3_t maxs,
				clipHandle_t model, int brushmask,
				const vec3_t origin, const vec3_t angles)
{
...
void
trap_R_AddPolysToScene(qhandle_t hShader, int numVerts, const polyVert_t *verts, int num)
{
	syscall(CG_R_ADDPOLYSTOSCENE, hShader, numVerts, verts, num);
}

int
trap_R_LightForPoint(vec3_t point, vec3_t ambientLight, vec3_t directedLight, vec3_t lightDir)
{
...
void
trap_R_AddAdditiveLightToScene(const vec3_t org, float intensity, float r, float g, float b)
{
...
int
trap_R_LerpTag(orientation_t *tag, clipHandle_t mod, int startFrame, int endFrame,
...
void
trap_SetUserCmdValue(int val0, int val1, int val2, float sensitivityScale)
{
	syscall(CG_SETUSERCMDVALUE, val0, val1, val2 PASSFLOAT(sensitivityScale));
}
...
int
trap_PC_AddGlobalDefine(char *define)
{
...
// this returns a handle.  arg0 is the name in the format "idlogo.roq", set arg1 to nil, alteredstates to qfalse (do not alter gamestate)
int
trap_CIN_PlayCinematic(const char *arg0, int xpos, int ypos, int width, int height, int bits)
{
