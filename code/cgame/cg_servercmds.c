...
*/
int
CG_ParseVoiceChats(const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats)
{
	int len, i;
	fileHandle_t f;
...
CG_LoadVoiceChats(void)
{
	...
	cgprintf("voice chat memory size = %d\n", size - trap_MemoryRemaining());
}
...
*/
int
CG_GetVoiceChat(voiceChatList_t *voiceChatList, const char *id, sfxHandle_t *snd, char **chat)
{
	int i, rnd;
...
CG_VoiceChat(int mode)
{
	...
	voiceOnly = atoi(cgargv(1));
	clientNum = atoi(cgargv(2));
	color = atoi(cgargv(3));
	cmd = cgargv(4);
...
CG_ServerCommand(void)
{
	...
	if(!strcmp(cmd, "print")){
		cgprintf("%s", cgargv(1));
#ifdef MISSIONPACK
		cmd = cgargv(1);	// yes, this is obviously a hack, but so is the way we hear about
