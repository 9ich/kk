...
*/
static qboolean
S_AL_GenBuffers(ALsizei numBuffers, ALuint *buffers, const char *name)
{
	ALenum error;
...
*/
static void
S_AL_BufferUse(sfxHandle_t sfx)
{
	if(knownSfx[sfx].filename[0] == '\0')
		return;
...
*/
static sfxHandle_t
S_AL_RegisterSound(const char *sample, qboolean compressed)
{
...
*/
static ALuint
S_AL_BufferGet(sfxHandle_t sfx)
{
...
static void
S_AL_Gain(ALuint source, float gainval)
{
	if(s_muted->integer)
...
static void
S_AL_ScaleGain(src_t *chksrc, vec3_t origin)
{
	float distance;
...
static void
S_AL_SaveLoopPos(src_t *dest, ALuint alSource)
{
	int error;
...
*/
static srcHandle_t
S_AL_SrcAlloc(alSrcPriority_t priority, int entnum, int channel)
{
	int i;
	int empty = -1;
	int weakest = -1;
...
#if 0
static srcHandle_t
S_AL_SrcFind(int entnum, int channel)
{
	int i;
	for(i = 0; i < srcCount; i++){
...
*/
static void
S_AL_SrcUnlock(srcHandle_t src)
{
...
*/
static void
S_AL_StartLocalSound(sfxHandle_t sfx, int channel)
{
	srcHandle_t src;
...
*/
static void
S_AL_ClearLoopingSounds(qboolean killall)
{
	int i;
	for(i = 0; i < srcCount; i++)
...
*/
static void
S_AL_RawSamples(int stream, int samples, int rate, int width, int channels, const byte *data, float volume, int entityNum)
{
	int numBuffers;
	ALuint buffer;
	ALuint format;
...
*/
static void
S_AL_StartBackgroundTrack(const char *intro, const char *loop)
{
	int i;
	qboolean issame;
...
*/
static void
S_AL_Respatialize(int entityNum, const vec3_t origin, const vec3_t vel, vec3_t axis[3], int inwater)
{
	float orientation[6];
	vec3_t sorigin, svel;
...
static int
S_AL_AvailableCaptureSamples(void)
{
	int retval = 0;
	if(alCaptureDevice != NULL){
		ALint samples = 0;
...
static void
S_AL_Capture(int samples, byte *data)
{
	if(alCaptureDevice != NULL)
